...
enum Kind { NOTHING, HULL, COMPONENT, ROCK }

## Returns `{"kind": Kind, "t": float, "point": Vector3, "component": int}`.
## `t` is the fraction along a→b at which the shot was stopped, -1 for nothing hit.
## `component` is the index of the component reached, or -1.
static func resolve(a: Vector3, b: Vector3, target: TargetShip,
		rocks: ReferenceField) -> Dictionary:
	...
	if kind == Kind.NOTHING:
		return {"kind": Kind.NOTHING, "t": -1.0, "point": b, "component": -1}
	return {"kind": kind, "t": best, "point": point, "component": component}
