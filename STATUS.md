...
`docs/EXPLORATION_DESIGN.md` (locked decisions), `docs/EXPLORATION_POC_IMPLEMENTATION.md`
(the build order, with ✅ marks on what is built), and **`docs/HIGHWAY.md`** (how the
road is built, authored and verified). Those documents **supersede prior decisions
where they conflict**, at the human's explicit direction.
...
Eight days of highway sessions (ADRs 0070–0095) produced a road whose collision was
arithmetic against a path with every opening declared as an exception, whose mesh was
independent of that collision by design, and whose every fix became an ADR with a
forbids list. The human gave a clean-room session a one-page requirement — *"a tileset
of square tubes that can connect and branch and allow a ship to move through with
bounce collision on the walls"* — and no direction on how. It came back with 2,600
lines, one idea (the mesh and the collision share the rule for where a wall is open),
and connectedness and collision the human found correct on the first drive, on and
off ramps included. That road is ported here and the old one is gone. **ADR 0096** is
the decision; **`docs/HIGHWAY.md`** is the mechanism.

What is kept from this project: the mothership, the camera, the berth, the strip, fuel,
docking, the boundary, the deep field, the tuning panel. What is ported: `RoadPath`
(straights and arcs, analytic), `Tube`, `Road`, `RoadCollider`, `RoadMesh`,
`RoadNetwork`, and the flying test suite. What is new here: the mesh streams in chunks
on worker threads around the ship (the POC built a whole map up front and took
minutes), the map is data in `data/routes.json` with a rule that builds every planet
ramp, and `make roads` validates it.

What is deleted: `RoadStructure`, `RoadDeck`, `HullBarrier`, `LaneProfile`,
`ExitSign`, the module generator and its assets, the lattice branches (never merged),
`HIGHWAY_STRUCTURE_PLAN.md`, `HIGHWAY_LATTICE_PLAN.md`, 35 tuning keys, thirteen ADRs,
and about 800 lines of this file that described them. `decisions/README.md` records
which.

**Nothing about the road is signed off from the seat.** The gate flies it; the human
...
- Interchange ramps X1–X4 validate and the probe flies them, but they are long
  (12–20 km) and authored by hand; nobody has flown them.
- The far version of a road (beyond `road_detail_radius`) has no openings, so a
  junction seen from far away looks closed until its chunk streams in.
- Entering a highway's own end mouths (the open ends of A-377B and K-112) works but
  they carry no portal and no label.
- The `road_shot` harness renders one spot; there is no shot of every junction.
