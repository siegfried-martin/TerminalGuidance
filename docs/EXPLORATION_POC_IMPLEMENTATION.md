...
2. **One system disc with a planet.** Existing manual flight. Nothing new to fly. *Note that the boundary treatment was **not** existing — see the amendment above; ADR 0011 was built for the first time here.* ✅ **built 2026-08-29**: `scenes/exploration.tscn`, `SystemDisc`, `Planet`, `BoundaryField`, run with `make run SCENE=res://scenes/exploration.tscn`.
...
5. **⬅ NEXT.** *(The system border landed first — ADR 0062, the rim is a boundary with funnel apertures — because the local leg is what an aperture opens onto. The single system already declares a placeholder aperture; attach the leg at `SystemDisc.aperture_mouth(0)` and replace `aperture_bearing_deg` with the leg's own bearing.)* **Second system and the local leg, off-road only.** No highway yet. Fly A to B manually at each hull speed. This establishes the baseline the highway must beat and is the control condition for success criterion 2.
