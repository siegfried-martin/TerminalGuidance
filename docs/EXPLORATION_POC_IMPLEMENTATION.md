...
5. **⬅ NEXT, after the system border** (see `STATUS.md` §SETTLED, NOT YET BUILT — the rim becomes a boundary with funnel apertures, and that lands first because the local leg is what the aperture opens onto). **Second system and the local leg, off-road only.** No highway yet. Fly A to B manually at each hull speed. This establishes the baseline the highway must beat and is the control condition for success criterion 2.
